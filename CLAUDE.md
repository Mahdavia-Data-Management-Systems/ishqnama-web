...
- **`pr-validation.yml`** — Runs on `pull_request` into `main`: frontend lint and build, backend Release build, `terraform fmt -check`. It is the only workflow a fork can trigger, so it holds no secrets, declares no environment and takes a read-only token; its three job names are required status checks on `main`
