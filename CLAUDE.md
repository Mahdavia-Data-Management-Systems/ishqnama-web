...
- **Sharing a verse**: the Share button on a verse (continuous-mode block or verse-mode popup) opens `src/components/scripture/share-verse-sheet.tsx`, which offers three places the link can open from: the chapter (`/quran/<c>/?verse=<v>`), the juz (`/quran/juz/<j>/?verse=<c>-<v>`, the `chapter-verse` form because verse numbers repeat across a juz and the reader's `?verse=` scroll already accepts it) and the ruku within the juz (`/quran/juz/<j>/ruku/<r>/?verse=<v>`). Juz and ruku numbers come from the reader's `rukuMap` (the verse's `RukuDto`), so those two options stay disabled until it loads. Link and message builders live in `src/lib/share-verse.ts`; the message opens with "Noor-e-Imaan | <chapter name> <c>:<v>", plus " | Juz <j>" or " | Juz <j>, Ruku <r>" when shared from the juz or ruku, then the translation as displayed on its own line, never the Arabic. The link travels separately: `navigator.share` gets it in `url` only (share targets append `url` to `text` themselves, so putting it in both printed it twice), and the clipboard fallback appends it once to the copied text, with a "Copied" confirmation in the sheet
